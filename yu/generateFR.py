...
files = glob('yu/cmi/*csv', recursive=True)
...
d17 = d[d.date.dt.year.isin([2017])].drop_duplicates(subset=['date'])

d18 = d[d.date.dt.year.isin([2018])].drop_duplicates(subset=['date'])

d = pd.concat([d17,d18])
plt.figure()
plt.plot(d.date, d.cmi.values)
...
d.to_csv('yu/cmi/FR.csv', index=False)
