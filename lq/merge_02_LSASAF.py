...
files = glob.glob('lq/lsasaf/*csv', recursive=True)
site = Site("LQ")

d = pd.concat([pd.read_csv(x)  for x in files])
d = d[['date','GHI']]

d.columns = ['date','lsasaf']
d['date'] = pd.to_datetime(d.date)
d = d.sort_values(by=['date'])

dates = pd.date_range(
    start="2020/01/01 00:00", end="2023/12/31 23:59", freq="15min")
...
meas = pd.read_csv('lq/meas/lq_15.csv')
